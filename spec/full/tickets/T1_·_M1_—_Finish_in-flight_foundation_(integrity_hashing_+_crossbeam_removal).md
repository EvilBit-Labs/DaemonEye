...
- Criterion baselines recorded for hashing impact. For the crossbeam removal, R14 AC4 requires either a no-regression comparison against a recorded pre-migration baseline on the daemoneye-eventbus broker end-to-end path (not the `EventBusConnector` WAL bench), **or** a recorded result and a deferral.

  _Status: neither was done, and the criterion as written does not cover what happened._ The removal proceeded on a third rationale — the bus was export-only dead code with no runtime consumers, so deleting it could not regress a live path. The hashing baselines and the broker benchmark exist; the pre-migration comparison number was never captured. Either add the dead-code exception to the criterion explicitly, or mark R14 AC4 incomplete and treat the removal as deferred. See **Remaining work** item 3.
