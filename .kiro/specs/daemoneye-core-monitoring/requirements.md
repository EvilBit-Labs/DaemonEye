...
6. WHEN rules contain REGEXP operators THEN the system SHALL compile patterns at rule load time using a linear-time regex engine (Rust regex crate) with compile-time size and complexity limits — eliminating catastrophic backtracking by construction — and SHALL cache compiled patterns keyed by the full pattern string with LRU eviction (bounds per "Regex Implementation Requirements" in `spec/daemon_eye_spec_sql_to_ipc_detection_architecture.md` §4.2). NOTE: §4.2's stated bounds — 1 MB per pattern against `cache_size: 1000` — permit roughly 1 GB, an order of magnitude over the \<100 MB resident budget in AGENTS.md. A compiled pattern's memory cannot be read back at runtime, so that product is the real ceiling and no aggregate cap can enforce a lower one. The bounds need reconciling against the budget before implementation fixes them
