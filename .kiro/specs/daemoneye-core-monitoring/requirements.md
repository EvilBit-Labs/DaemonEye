...
6. WHEN rules contain REGEXP operators THEN the system SHALL compile patterns at rule load time using a linear-time regex engine (Rust regex crate) with compile-time size and complexity limits — eliminating catastrophic backtracking by construction — and SHALL cache compiled patterns keyed by the full pattern string with LRU eviction (bounds per "Regex Implementation Requirements" in `spec/daemon_eye_spec_sql_to_ipc_detection_architecture.md` §4.2: 256KiB per pattern against a 64-entry cache, a 16MiB product). Backreferences and lookaround are rejected at load — a linear-time engine cannot compile them
