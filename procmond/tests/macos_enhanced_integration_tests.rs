//! Enhanced macOS-specific integration tests using third-party crates.
//!
//! This module contains integration tests that verify the enhanced macOS-specific
//! process collector functionality using third-party crates instead of direct
...
mod macos_enhanced_tests {
    ...
    /// Test enhanced macOS collector creation and configuration.
    #[tokio::test]
    #[traced_test]
    ...
    /// Test enhanced macOS collector capabilities with different configurations.
    #[tokio::test]
    #[traced_test]
    ...
    /// Test enhanced macOS collector health check functionality.
    #[tokio::test]
    #[traced_test]
    ...
    /// Test enhanced macOS process collection using third-party crates.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_process_collection() {
        let base_config = ProcessCollectionConfig {
    ...
    /// Test enhanced macOS single process collection.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_single_process_collection() {
        let base_config = ProcessCollectionConfig::default();
        let macos_config = MacOSCollectorConfig::default();
        let collector = EnhancedMacOSCollector::new(base_config, macos_config).unwrap();
        ...
        println!(
            "Successfully collected current process using third-party crates: PID={}, name={}",
            event.pid, event.name
        );
    }

    /// Test enhanced macOS collector with Security framework integration.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_security_framework_integration() {
        let base_config = ProcessCollectionConfig {
            collect_enhanced_metadata: true,
    ...
    /// Test enhanced macOS collector error handling for non-existent process.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_nonexistent_process() {
        let base_config = ProcessCollectionConfig::default();
        let macos_config = MacOSCollectorConfig::default();
        let collector = EnhancedMacOSCollector::new(base_config, macos_config).unwrap();
    ...
    /// Test enhanced macOS collector with system process filtering.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_system_process_filtering() {
        let base_config = ProcessCollectionConfig {
            skip_system_processes: true,
            max_processes: 20,
            ..Default::default()
        ...
        println!(
            "Enhanced collector: collected {} processes, {} inaccessible (filtered)",
    ...
    /// Test enhanced macOS collector performance under load.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_collector_performance() {
        let base_config = ProcessCollectionConfig {
            collect_enhanced_metadata: true,
        ...
        println!(
            "Enhanced collector performance test: collected {} processes in {}ms (total: {}ms)",
            events.len(),
            stats.collection_duration_ms,
    ...
    /// Test enhanced macOS collector SIP detection functionality.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_sip_detection() {
        let base_config = ProcessCollectionConfig::default();
        let macos_config = MacOSCollectorConfig {
            check_sip_protection: true,
            ..Default::default()
        ...
        assert!(
            collector.is_ok(),
            "Enhanced collector creation should succeed even with SIP detection enabled"
        );
    ...
    /// Test enhanced macOS collector entitlements detection.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_entitlements_detection() {
        let base_config = ProcessCollectionConfig::default();
        let macos_config = MacOSCollectorConfig {
            collect_entitlements: true,
            ..Default::default()
        ...
        assert!(
            collector.is_ok(),
            "Enhanced collector creation should succeed even with entitlements detection enabled"
        );
    ...
    /// Test enhanced macOS collector graceful degradation.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_graceful_degradation() {
        ...
        println!(
            "Enhanced collector graceful degradation test: collected {} processes with minimal configuration",
            events.len()
        );
    }

    /// Test enhanced macOS collector third-party crate integration.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_macos_third_party_integration() {
        let base_config = ProcessCollectionConfig {
            collect_enhanced_metadata: true,
            max_processes: 20,
            ..Default::default()
        ...
        // Verify we can collect processes using third-party crates
        assert!(
            !events.is_empty(),
            "Should collect processes using third-party crates"
        );
        assert!(
            stats.successful_collections > 0,
            "Should have successful collections"
        );
        ...
        println!(
            "Third-party crate integration test: collected {} processes successfully",
            events.len()
        );
    }

    /// Test enhanced macOS collector comparison with original implementation.
    #[tokio::test]
    #[traced_test]
    async fn test_enhanced_vs_original_macos_collector() {
        ...
        let enhanced_collector =
            EnhancedMacOSCollector::new(base_config.clone(), macos_config.clone()).unwrap();
        let (enhanced_events, enhanced_stats) =
        ...
        // Both should collect processes successfully
        assert!(
            !enhanced_events.is_empty(),
            "Enhanced collector should collect processes"
        );
        assert!(
            !original_events.is_empty(),
            "Original collector should collect processes"
        );

        // Both should have reasonable success rates
        let enhanced_success_rate =
            enhanced_stats.successful_collections as f64 / enhanced_stats.total_processes as f64;
        let original_success_rate =
        ...
        println!(
            "Collector comparison: Enhanced={} processes ({}ms), Original={} processes ({}ms)",
            enhanced_events.len(),
            enhanced_stats.collection_duration_ms,
            original_events.len(),
            original_stats.collection_duration_ms
        );
    }
}
...
mod non_macos_tests {
    ...
    #[test]
    fn test_enhanced_macos_tests_skipped_on_non_macos() {
        println!("Enhanced macOS-specific tests are skipped on non-macOS platforms");
    }
}
